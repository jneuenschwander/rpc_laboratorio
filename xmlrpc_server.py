...
class RPC:
    ...
    def post(self, data, usuario):
        ...
        file.close()
        f = open('post.txt', 'a')
        f.write('\n' + str(data['ID']) + ',' + data['Titular'] + ',' + data['fechacreacion'] + ',' + data[
            'fechaactualizacion'] + ',' + data['Autor'] + ',' + data['Contenido'])
        f.close()
    ...
    def edit(self, datas, user):
        ...
        with open('post.txt', 'r') as f:
            ...
            if len(data) == 0:
                ...
                if (int(dataIndex) == index and dataUser == user) or (
                        int(dataIndex) == index and user == 'admin'):  # corregir
